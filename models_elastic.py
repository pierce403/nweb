...
def search(query, limit, offset):
  ...
  try:
    ...
    result = es.search(index=host_index, body={'size':limit, 'from':offset, 'timeout':3000, 'track_total_hits':False, 'query':{'query_string': {'query':query, 'default_operator':'AND'}},  'sort':{'timestamp': {'order': 'desc'}}})
  except Exception as e:
    print('[E] ' + str(e))
    return 0, []
